...
class Tank_auth
{
	...
	function create_user($username, $firstname, $lastname, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('user_name' => 'auth_username_in_use');
	...
	function update_user($username, $firstname, $lastname, $email, $password, $email_activation,$user_id)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('user_name' => 'auth_username_in_use');
			...
			if (!is_null($res = $this->ci->users->update_user($data, !$email_activation))) {
				$data['password'] = $password;
				unset($data['last_ip']);
				return $data;
			}
	 ...
	 * @return	array
	 */
	function create_user_oa2($email, $provider, $firstname, $lastname,$img_url = '')
	{
	...
	function update_user_oa2($email, $provider, $firstname, $lastname,$img_url = '',$user_id)
	{
		if (!$this->ci->users->is_email_available($email)) {
			$this->error = array('email' => 'auth_email_in_use');
			...
			if (!is_null($res = $this->ci->users->update_user($data))) {
				unset($data['last_ip']);
				return $data;
			}
		}
