...
class OAuth2_Token_Access extends OAuth2_Token
{
	...
	public function __construct(array $options = null)
	{
		if(empty($options['access_token'])) {
			$formatted_response = json_decode(array_keys($options)[0]);
			$options['access_token'] = $formatted_response->access_token;
			$options['token_type'] = $formatted_response->token_type;
			$options['expires_in'] = $formatted_response->expires_in;	
		}
